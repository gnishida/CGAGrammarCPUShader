...
void GableRoof::generateGeometry(float opacity, std::vector<std::vector<Vertex> >& vertices) const {
	Polygon_2 poly;
	for (int i = 0; i < _points.size(); ++i) {
		poly.push_back(KPoint(_points[i].x, _points[i].y));
	}
	...
	for (auto face = iss->faces_begin(); face != iss->faces_end(); ++face) {
		...
		do {
			...
			if (first) {
				...
				pts.push_back(glm::vec3(p0, 0));
				pts.push_back(glm::vec3(p1, 0));
			} else {
				glm::vec2 p2 = glm::vec2(head->point().x(), head->point().y());
				...
				if (p2 != p0) {
					...
					vertices[face_id].push_back(Vertex(v0, normal, glm::vec4(_color, opacity)));
					vertices[face_id].push_back(Vertex(v1, normal, glm::vec4(_color, opacity)));
					vertices[face_id].push_back(Vertex(v2, normal, glm::vec4(_color, opacity)));*/
		...
		glutils::drawPolygon(pts, glm::vec4(_color, opacity), _pivot * _modelMat, vertices[offset]);
	}
}
