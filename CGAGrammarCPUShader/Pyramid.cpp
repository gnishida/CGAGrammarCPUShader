...
void Pyramid::generateGeometry(float opacity, std::vector<std::vector<Vertex> >& vertices) const {
	if (_removed) return;
	...
	if (_top_ratio == 0.0f) {
		...
		for (int i = 0; i < _points.size(); ++i) {
			...
			vertices[offset + i].push_back(Vertex(glm::vec3(p0), normal, glm::vec4(_color, opacity), glm::vec2(0, 0)));
			vertices[offset + i].push_back(Vertex(glm::vec3(p1), normal, glm::vec4(_color, opacity), glm::vec2(0, 0)));
			vertices[offset + i].push_back(Vertex(glm::vec3(p2), normal, glm::vec4(_color, opacity), glm::vec2(0, 0)));
		...
		for (int i = 0; i < _points.size(); ++i) {
			...
			vertices[offset + i].push_back(Vertex(glm::vec3(p0), normal, glm::vec4(_color, opacity), glm::vec2(0, 0)));
			vertices[offset + i].push_back(Vertex(glm::vec3(p2), normal, glm::vec4(_color, opacity), glm::vec2(0, 0)));
			vertices[offset + i].push_back(Vertex(glm::vec3(p3), normal, glm::vec4(_color, opacity), glm::vec2(0, 0)));
			vertices[offset + i].push_back(Vertex(glm::vec3(p1), normal, glm::vec4(_color, opacity), glm::vec2(0, 0)));
		...
		glutils::drawPolygon(pts3, glm::vec4(_color, opacity), _pivot * _modelMat, vertices[offset]);
	}
}
