...
void Prism::generateGeometry(float opacity, std::vector<std::vector<Vertex> >& vertices) const {
	if (_removed) return;
	...
	if (_scope.z >= 0) {
		...
		glutils::drawPolygon(_points, glm::vec4(_color, opacity), mat, vertices[offset++]);
	}

	// bottom
	{
		vertices.resize(offset + 1);
		glutils::drawPolygon(_points, glm::vec4(_color, opacity), _pivot * _modelMat, vertices[offset++]);
	}
	...
	// side
	{
		...
		vertices.resize(offset + _points.size());
		for (int i = 0; i < _points.size(); ++i) {
			...
			vertices[offset + i].push_back(Vertex(glm::vec3(p1), normal, glm::vec4(_color, opacity)));
			vertices[offset + i].push_back(Vertex(glm::vec3(p3), normal, glm::vec4(_color, opacity), 1));
			vertices[offset + i].push_back(Vertex(glm::vec3(p4), normal, glm::vec4(_color, opacity)));
			vertices[offset + i].push_back(Vertex(glm::vec3(p2), normal, glm::vec4(_color, opacity), 1));
