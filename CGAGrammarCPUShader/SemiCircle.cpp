...
void SemiCircle::generateGeometry(float opacity, std::vector<std::vector<Vertex> >& vertices) const {
	if (_removed) return;
	...
	for (int i = 0; i < numSlices; ++i) {
		...
		vertices[offset].push_back(Vertex(p0, normal, glm::vec4(_color, opacity)));
		if (i < numSlices) {
			vertices[offset].push_back(Vertex(glm::vec3(p1), normal, glm::vec4(_color, opacity), 1));
		} else {
			vertices[offset].push_back(Vertex(glm::vec3(p1), normal, glm::vec4(_color, opacity)));
		}
		if (i > 0) {
			vertices[offset].push_back(Vertex(glm::vec3(p2), normal, glm::vec4(_color, opacity), 1));
		} else {
			vertices[offset].push_back(Vertex(glm::vec3(p2), normal, glm::vec4(_color, opacity)));
		}
	}
}
