...
void HipRoof::generateGeometry(float opacity, std::vector<std::vector<Vertex> >& vertices) const {
	Polygon_2 poly;
	...
	for (auto face = iss->faces_begin(); face != iss->faces_end(); ++face) {
		...
		int face_id = vertices.size();
		vertices.resize(face_id + 1);
		for (int i = 0; i < points.size(); ++i) {
			vertices[face_id].push_back(Vertex(points[i], normal, glm::vec4(_color, opacity)));
		}
